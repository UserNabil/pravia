...
import { ORDER_STATUSES, ORDER_STATUS_LABELS, ORDER_STATUS_STYLES } from "@/lib/constants";
    ...
    <div>
      ...
      {nouvelle === "1" && (
        <div className="mt-3 flex items-start gap-3 rounded-xl border border-success/30 bg-success/10 p-4">
          ...
          <div>
            <p className="text-sm font-semibold text-success">Commande confirmee</p>
            <p className="mt-0.5 text-sm text-muted">
              Merci ! Un e-mail de confirmation vous a ete envoye. Vous pouvez suivre l&apos;avancement
              ci-dessous.
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-2">Passee le {formatDateTime(order.createdAt)}</p>
        </div>
      ...
      {!isCancelled && (
        <section className="surface-card mt-5 p-5">
          ...
          {order.trackingNumber && (
            <p className="mt-5 flex items-center gap-2 rounded-lg bg-surface-2 p-3 text-sm">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.5fr_1fr] lg:items-start">
        ...
        <div className="space-y-5">
          <section className="surface-card p-5">
            ...
            <p className="mt-3 text-xs text-muted-2">Regle par {order.paymentMethod}</p>
          </section>
          ...
          <section className="surface-card p-5">
            ...
            <address className="mt-3 space-y-0.5 text-sm not-italic text-muted">
              ...
              {order.shipPhone && <p className="pt-1">{order.shipPhone}</p>}
            </address>
          </section>
