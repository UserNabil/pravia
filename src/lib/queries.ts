...
import { resolveCategory, resolveProduct, translationFilter } from "./content";
...
export type CatalogFilters = {
  ...
  tokens?: string[];
  /** Langue d'affichage : seuls les libelles changent, jamais le filtrage. */
  locale?: Locale;
};
 ...
 */
export async function findFallbackProducts(tokens: string[], take = 4, locale: Locale = "fr") {
...
export async function getProductBySlug(slug: string, locale: Locale = "fr") {
  ...
  // Les libelles suivent la langue ; les valeurs techniques restent telles quelles.
  const product = {
    ...resolveProduct(row),
    category: resolveCategory(row.category),
    specs: row.specs.map((spec) => ({
      ...spec,
      label: spec.translations?.[0]?.label?.trim() || spec.label,
      value: spec.translations?.[0]?.value?.trim() || spec.value,
    })),
  };
...
export async function getFacets(locale: Locale = "fr") {
  ...
  return {
    categories: categories.map(resolveCategory),
...
export async function getCartWithProducts(userId: string, locale: Locale = "fr") {
  ...
  // Le titre affiche suit la langue : le reste du panier n'a pas a le savoir.
  const items = rows.map((item) => ({ ...item, product: resolveProduct(item.product) }));
