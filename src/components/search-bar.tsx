  ...
  return (
    <div ref={containerRef} className={cn("relative", className)}>
      <form
        ...
        role="search"
        className="flex h-10 w-full items-center rounded-full border border-border bg-surface-2 pe-1.5 transition-colors focus-within:border-primary"
      >
        <div className="relative hidden sm:block">
          <button
            ...
            className="ms-1 flex h-8 items-center gap-1 rounded-full bg-primary px-3 text-xs font-semibold text-primary-foreground"
          >
            <span className="max-w-28 truncate">{activeCategory?.name ?? t("products")}</span>
          ...
          {categoryOpen && (
            <ul
              role="listbox"
              className="absolute start-0 top-10 z-50 max-h-80 w-60 overflow-auto rounded-xl border border-border bg-surface p-1 shadow-xl shadow-black/20"
            >
              <li>
                <button
                  type="button"
                  ...
                  className={cn(
                    "w-full rounded-lg px-3 py-2 text-start text-sm transition-colors hover:bg-surface-2",
        ...
        {loading && <Loader2 className="me-1 size-3.5 shrink-0 animate-spin text-muted-2" />}
        ...
        {term && !loading && (
          <button
            ...
            aria-label={t("clear")}
            className="me-1 flex size-6 shrink-0 items-center justify-center rounded-full text-muted-2 transition-colors hover:text-foreground"
          >
        ...
        <div className="absolute inset-x-0 top-12 z-50 overflow-hidden rounded-xl border border-border bg-surface shadow-xl shadow-black/20">
            ...
            <div className="p-1.5">
              ...
              {history.map((entry) => (
                <button
                  ...
                  className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-start text-sm transition-colors hover:bg-surface-2"
                >
              ...
              {suggestions.categories.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="ms-auto shrink-0 text-xs text-muted-2">{t("category")}</span>
                  </button>
                );
              })}
              ...
              {suggestions.brands.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <Tag className="size-3.5 shrink-0 text-muted-2" />
                    <span className="truncate">{item.name}</span>
                    <span className="ms-auto shrink-0 text-xs text-muted-2">{t("brand")}</span>
                  </button>
                );
              })}
            </div>
          ...
          {term.trim().length >= 2 && (
            <button
              type="button"
              onClick={() => submit()}
              className="w-full border-t border-border px-3 py-2.5 text-start text-sm font-medium text-primary transition-colors hover:bg-surface-2"
            >
              {t("seeAllResults", { term: term.trim() })}
            </button>
          )}
        </div>
      )}
